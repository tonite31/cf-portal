		...
		$('#spaceName span').on('click', function()
		{
			...
			$(this).on('blur', function()
			{
				if(prev != $(this).text())
				{
					...
					$('.space-name-description').text('Please, wait for update.').css('color', '#337ab7');
					CF.async({url : '/v2/spaces/' + space.metadata.guid, method : 'PUT', form : {name : $(this).text()}}, function(result)
					{
						...
						{
							$('#spaceName span:first').text(prev);
							$('.space-name-description').text('Unknown error.').css('color', '');
						}
					},
					function(error)
					{
						$('#spaceName span:first').text(prev);
						$('.space-name-description').text(error.stack ? error.stack : error).css('color', '');
					});
				}
